...
class PromptBuilder:
    ...
    def _build_prompt(self, mode, prompt, fpath, top_k_context):
        ...
        return prepend_context + f'Based on the information above, please complete the function in the current file {fpath}:' + '\n' + prompt, chosen_context
    ...
    def build_2nd_stage_input_file(self, mode):
        ...
        for query_line in self.query_lines_with_retrieval_results:
            ...
            fpath = '/'.join(query_line['metadata']['fpath_tuple'][1:])
            new_prompt, chosen_context = self._build_prompt(mode, old_prompt, fpath, top_k_context)
            new_prompt_line = {
                'prompt': new_prompt,
...
class BuildPromptWrapper:
    ...
    def _run(self, mode, query_window_path_builder, output_file_path):
        ...
        for repo in self.repos:
            ...
            repo_window_path = FilePathBuilder.repo_windows_path(self.repo_base_dir, repo, self.window_size, self.slice_size)
    ...
    def build_first_search_prompt(self, mode, output_path):
        query_line_path_temp = functools.partial(FilePathBuilder.search_first_window_path, self.repo_base_dir, mode)
        self._run(mode, query_line_path_temp, output_path)

    
    def build_prediction_prompt(self, mode, prediction_path, output_path):
        query_line_path_temp = functools.partial(FilePathBuilder.gen_first_window_path, self.repo_base_dir, mode, prediction_path)
        self._run(mode, query_line_path_temp, output_path)
