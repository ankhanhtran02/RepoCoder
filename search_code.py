...
class CodeSearchWrapper:
    def __init__(self, vectorizer, benchmark, repos, window_sizes, slice_sizes, repo_base_dir):
        ...
        for window_size in self.window_sizes:
            for slice_size in self.slice_sizes:
                for repo in self.repos:
                    ...
                    repo_window_path = FilePathBuilder.repo_windows_path(self.repo_base_dir, repo, window_size, slice_size)
    ...
    def search_baseline_and_ground(self):
        query_line_path_temp = functools.partial(FilePathBuilder.search_first_window_path, self.repo_base_dir, CONSTANTS.rg)
        self._run_parallel(query_line_path_temp)
        query_line_path_temp = functools.partial(FilePathBuilder.search_first_window_path, self.repo_base_dir, CONSTANTS.gt)
        self._run_parallel(query_line_path_temp)
    
    def search_prediction(self, mode, prediction_path_template):
        query_line_path_temp = functools.partial(FilePathBuilder.gen_first_window_path, self.repo_base_dir, mode)
