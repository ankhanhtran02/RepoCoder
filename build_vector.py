...
class BuildVectorWrapper:
    def __init__(self, benchmark, vector_builder, repos, window_sizes, slice_sizes, repo_base_dir=FilePathBuilder.repo_base_dir):
        self.repos = repos
        ...
        for window_size, slice_size in itertools.product(self.window_sizes, self.slice_sizes):
            for repo in self.repos:
                builder = self.vector_builder(
                    FilePathBuilder.repo_windows_path(self.repo_base_dir, repo, window_size, slice_size)
                )
                builder.build()
...
class BuildEmbeddingVector:
    ...
    '''
    def __init__(self, repos, window_sizes, slice_sizes, repo_base_dir=FilePathBuilder.repo_base_dir):
        self.repos = repos
        self.window_sizes = window_sizes
        self.slice_sizes = slice_sizes
        self.repo_base_dir = repo_base_dir
    ...
    def build_input_file_for_repo_window(self, slice_size):
        ...
        for window_size in self.window_sizes:
            for repo in self.repos:
                file_path = FilePathBuilder.repo_windows_path(self.repo_base_dir, repo, window_size, slice_size)
    ...
    def build_input_file_search_first_window(self, mode, benchmark):
        ...
        for window_size in self.window_sizes:
            for repo in self.repos:
                file_path = FilePathBuilder.search_first_window_path(self.repo_base_dir, mode, repo, window_size)
                loaded_lines = Tools.load_pickle(file_path)
                for line in loaded_lines:
                    lines.append({
                        'context': line['context'],
    ...
    def build_input_file_for_gen_first_window(self, mode, benchmark, prediction_path):
        ...
        for window_size in self.window_sizes:
            for repo in self.repos:
                file_path = FilePathBuilder.gen_first_window_path(self.repo_base_dir, benchmark, mode, prediction_path, repo)
                loaded_lines = Tools.load_pickle(file_path)
                for line in loaded_lines:
                    lines.append({
                        'context': line['context'],
